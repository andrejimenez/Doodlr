...
def p_agregaFuncion(p):
    ''' agregaFuncion : '''
    funciones.append(EstrucFunc(p[-1], tipoDeVar, len(cuadruplos)))
    ...
    lFloat = 0
    lBool = 0
    dire = setDireccionMem(tipoDeVar,0,0,0,0,0,0)
    funciones[1].varTable.append(Var(p[-1], tipoDeVar, dire, None))
...
def p_funcIgual(p):
    ''' funcIgual : ID pushPilaVar OP_EQUALS pushPilaOp defExpresiones TO_PuntoComa extraNeed agregaIgualCuad
                  | ID pushPilaVar OP_EQUALS pushPilaOp llamadaDeFunciones agregaIgualFuncCuad'''
...
def p_decParamFuncs(p):
    ''' decParamFuncs : miniExp paramCuadruplo
...
def p_agregaIgualFuncCuad(p):
    ...
    if len(pilaOpp)> 0:
        if pilaOpp[len(pilaOpp)-1] == '=' :
            asigna = getDireccionMem(ID)
            operador = pilaOpp.pop()
            cuadruplos.append(cuadruplo(len(cuadruplos), "igual", None, None , asigna))
